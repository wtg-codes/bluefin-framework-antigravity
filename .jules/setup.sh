...
# 1. CNCF & OS Tooling (BATS, Just, Pipx, Yamllint)
# Jules needs BATS to run tests/os_validation.bats and Just to simulate ujust
echo "🛠️ Installing OS dependencies (bats, curl, jq, pipx, yamllint)..."
sudo apt-get update -y
sudo apt-get install -y bats curl jq pipx yamllint

if ! command -v just &> /dev/null; then
    curl --proto '=https' --tlsv1.2 -sSf https://just.systems/install.sh | sudo bash -s -- --to /usr/local/bin
fi

# 2. Node.js Version Update (Node 24 mandated by AGENTS.md)
echo "🟢 Setting Node.js to version 24..."
export NVM_DIR="$HOME/.nvm"
if [ -s "$NVM_DIR/nvm.sh" ]; then
    # Load NVM without aborting if it fails
    set +e
    . "$NVM_DIR/nvm.sh"
    nvm install 24
    nvm use 24
    ...
    echo "⚠️ NVM not found at $NVM_DIR, attempting fallback Node setup."
    # Optional fallback if NVM is truly missing, but it's typically present on standard Jules VMs
    curl -fsSL https://deb.nodesource.com/setup_24.x | sudo -E bash -
    sudo apt-get install -y nodejs
fi

# 3. Docusaurus Website Dependencies
echo "📦 Installing Docusaurus dependencies for /website..."
if [ -d "website" ]; then
    # Use --prefix to avoid fragile directory changes
    npm ci --prefix website
fi

# 4. AI Agent Dependencies (Gemini CLI & Google ADK)
# Mirrors the steps in files/workspace/Containerfile
echo "🤖 Setting up Google ADK and Gemini CLI..."
# Ensure pipx path is available
export PATH="$PATH:$HOME/.local/bin"
pipx install google-adk --pip-args="--retries 10" || echo "google-adk already installed or failed."
npm install -g @google/gemini-cli

# 5. GCP & API Configuration (Optional / Conditional)
...
# 6. Local Validation Pre-flight Check
